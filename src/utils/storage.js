...
const localStorageImpl = {
  ...
  getAllCorrectInputs: () => {
    return Object.keys(localStorage).filter((key) =>
      key.startsWith(`${STORAGE_PREFIX}input_correct_`)
    );
  },
...
export const isTaskCorrect = (id) =>
  storage.isTaskCorrect(id);

export const saveCorrectAnswer = (id) =>
  storage.saveCorrectAnswer(id);

export const migrateOldProgress = () =>
  storage.migrateOldProgress();

export const clearAnswersByIds = (ids) =>
  storage.clearAnswersByIds(ids);

export const getTaskKey =
  localStorageImpl.getTaskKey;
